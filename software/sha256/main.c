...
{
	if((mcause & (1 << RISCY_MCAUSE_INTERRUPT_BIT)) && (mcause & (1 << RISCY_MCAUSE_IRQ_BIT)))
	{
		...
		switch(irq)
		{
			...
			case PLATFORM_IRQ_BUS_ERROR:
			{
				...
				riscy_disable_interrupts();
				while(1) riscy_wfi();

				break;
			}
			default:
				riscy_disable_irq(irq);
				break;
		}
	}
}
...
int main(void)
{
	...
	// Enable interrupts:
	riscy_enable_irq(PLATFORM_IRQ_TIMER0);
	riscy_enable_irq(PLATFORM_IRQ_TIMER1);
	riscy_enable_irq(PLATFORM_IRQ_BUS_ERROR);
	riscy_enable_interrupts();
